...
Skill 执行后会返回 CLI 的 stdout。AI Agent 应根据输出内容向用户做自然语言转述：

- 包含 `"status": 200` 或 `"status": "success"` → 告知用户提交成功
- 包含 `"status": 402` 或 `"upgradeUrl"` → **友好提示**：用户需要订阅才能提交，并附上订阅页面链接
- 包含 `"status": 401` → 告知 Token 无效，引导重新登录
- 包含 `"status": 400` → 直接告知具体错误原因
